...
// 下单码只允许中文、英文字母和数字；ASCII 字母统一按小写比较。
...
// 该实现必须与 src/storefront/loginValidation.js 中的同名函数保持完全一致。
...
// 校验的是传入值本身（不在此处规范化）：请求头路径（src/plugins/auth.js）与
// scripts/provision-superadmin.js 都是“先 normalizeOrderCode 再 validateOrderCode”，
// 所以空白已被剔除；POST /api/v1/auth/login（src/routes/config.js）沿用“先校验原始值、
// 再规范化”的既有契约，原始值含空格仍返回 ORDER_CODE_INVALID_FORMAT（见 test/config.test.js）。
export function validateOrderCode(value) {
  if (typeof value !== 'string' || value.length === 0) return '请输入下单码';
  if (Array.from(value).length > MAX_ORDER_CODE_LENGTH) return `下单码最多 ${MAX_ORDER_CODE_LENGTH} 个字符`;
  if (!ORDER_CODE_PATTERN.test(value)) return '下单码只能使用中文、英文字母和数字，不能包含空格或其他符号';
  return null;
}

export function displayNameFromOrderCode(code) {
  const chinese = code.match(/^[\u3400-\u4dbf\u4e00-\u9fff]+/u)?.[0];
  return chinese || code;
}
